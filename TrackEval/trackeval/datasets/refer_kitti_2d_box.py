...
class ReferKitti2DBox(_BaseDataset):
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        """
        if cls == 'car+pedestrian':
            cls_id = [self.class_name_to_class_id['car'], self.class_name_to_class_id['pedestrian']]
        else:
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            tracker_class_mask = np.atleast_1d(np.isin(raw_data['tracker_classes'][t], cls_id))
            tracker_class_mask = tracker_class_mask.astype(np.bool)
            if raw_data['tracker_classes'][t] is None:
                tracker_class_mask = []
            tracker_ids = raw_data['tracker_ids'][t][tracker_class_mask]
            tracker_dets = raw_data['tracker_dets'][t][tracker_class_mask]
            similarity_scores = raw_data['similarity_scores'][t][:, tracker_class_mask]
