import pandas as pd
import torch
from src.data.seq_processor import KITTISeqProcessor
...
class KITTISceneDataset:
    ...
    def _load_seq_dfs(self):
        ...
        for dataset_path, seq_list in self.seqs.items():
            for seq_name in seq_list:
                ...
                seq_processor = KITTISeqProcessor(dataset_path=dataset_path, seq_name=seq_name, config=self.config)
    ...
    def _load_precomputed_embeddings(self, det_df, seq_info_dict, embeddings_dir):
        ...
        embeddings_path = osp.join(training_path, 'processed_data', seq_info_dict['seq'], 'embeddings',
