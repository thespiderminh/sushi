...
_WEIGHTS_DIR = osp.join(root,'fast-reid')
...
def _load_ckpt(model, cfg):
    ...
    ckpt=torch.load(cfg.MODEL.WEIGHTS, map_location = torch.device("cuda:2"))
...
#def get_mo
def load_fastreid_model(reid_arch_name, device):
    ...
    cfg.MODEL.DEVICE = "cuda:" + str(device.index)
